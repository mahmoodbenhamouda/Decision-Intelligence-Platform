...
# Tables actually needed by the warehouse layer
# Excluded very large movement/detail files (>100MB) for performance
REQUIRED_TABLES = {
    # Devis (estimations)
    "devis_vente_ent_vv",
    "devis_achat_ent_v",
    ...
    # Fournisseurs
    "fournisseurs_v",
    # Bons de livraison (entêtes seulement)
    "gsl_vente_bl_entete",
    "gsl_achat_bl_entete",
    # Factures GSL
    "gsl_vente_fa_entete",
    ...
    # Comparatifs achats
    "ms_comparatif_achat",
    "ms_comparatif_achat_facture",
    # Autres
    "ol_gcf_facture_achat_base",
    "table_etat_devis_technique_v",
}
...
def build_dim_fournisseur(tables: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if "fournisseurs_v" in tables:
        df_f = tables["fournisseurs_v"]
        avail_cols = [c for c in ['cle_fournisseur', 'nomfournisseur', 'nom', 'intitule'] if c in df_f.columns]
        if 'cle_fournisseur' in avail_cols:
            rename_map = {c: 'supplier_name' for c in avail_cols if c != 'cle_fournisseur'}
            candidates.append(df_f[avail_cols].rename(columns=rename_map))
